...
- [x] Cloner le repo et explorer l'arborescence (`models/`, `ollama_server/`, `tritton_server/`)
- [x] Choisir une solution d'inférence (recommandé : **Ollama** pour gagner du temps)
- [x] Installer Ollama : [ollama.com/download](https://ollama.com/download)
- [x] Créer le modèle depuis `ollama_server/Modelfile` :
  ```bash
  cd ollama_server
...
- [x] Tester l'API avec `curl` ou Postman (endpoint `/api/chat`)
- [x] Communiquer URL + port à P2 dès que le serveur est opérationnel
- [x] Documenter le choix technique et les paramètres d'inférence
...
- [x] Attendre l'URL du serveur INFRA (bloquant : ne pas démarrer l'intégration sans ça)
- [x] Choisir une stack : **Streamlit** (rapide), Flask, ou HTML/JS
- [x] Développer une interface de chat avec :
  - zone de saisie utilisateur
  - affichage de l'historique de conversation
  - indicateur d'état serveur (connecté / déconnecté)
  - gestion des erreurs réseau
- [x] Intégrer l'API du serveur choisi :
  - Ollama : `POST http://localhost:11434/api/chat`
  - Triton : `http://localhost:8000`
  - Serveur maison : URL fournie par P1
- [x] Permettre le lancement en **une seule commande** depuis `rendu/devweb/`
- [x] Tester bout en bout avec P1 avant la présentation
...
- [x] Tester le modèle via `scripts/simple_chat.py` ou le serveur déployé par P1
- [x] Poser **10+ questions financières** et noter les réponses (qualité, pertinence, hallucinations)
- [x] Évaluer : le modèle est-il fiable ? Déployable en l'état ?
- [x] Proposer des optimisations de paramètres (temperature, top_p, max_tokens) à P1
- [x] Partager un rapport de validation à P4 pour les tests de robustesse

#### Tâches — R&D (modèle médical)

- [x] Analyser les datasets dans `datasets/` (formats, volume, anomalies)
- [x] Télécharger le dataset médical : [ruslanmv/ai-medical-chatbot](https://huggingface.co/datasets/ruslanmv/ai-medical-chatbot)
- [x] Écrire un script Python d'analyse et de nettoyage des données
- [x] Préparer le dataset au format attendu pour le fine-tuning LoRA
- [x] Lancer le fine-tuning sur **Google Colab** (voir `medical_project/Readme.md`)
- [x] Partager le lien Colab + métriques (loss, epochs, échantillons de réponses)
...
- [x] Auditer tout l'héritage de l'équipe précédente :
  - code (`scripts/`, `model_repository/`)
  - logs (`logs/training.log`, `logs/team_logs_archive.md`)
  - datasets (`datasets/`)
- [x] Identifier les problèmes de sécurité et évaluer leur criticité
- [x] Tester la robustesse du modèle financier :
  - prompt injection
  - tentatives d'extraction de données sensibles
  - phrases déclencheuses suspectes (voir les logs)
- [x] Valider l'intégrité des réponses en production
- [x] Tester le modèle médical fine-tuné (une fois disponible via P3)
- [x] Vérifier l'absence de biais problématiques
...
- [x] Serveur d'inférence opérationnel (P1)
- [x] Interface web de chat fonctionnelle (P2)
- [x] Historique de conversation affiché (P2)
- [x] État de connexion serveur visible (P2)
- [x] Modèle financier testé sur 10+ questions (P3)
- [x] Audit sécurité réalisé avec rapport (P4)

### Expérimental (R&D)

- [x] Dataset médical analysé et nettoyé (P3)
- [x] Fine-tuning LoRA lancé sur Colab (P3)
- [x] Métriques d'entraînement documentées (P3)

### Bonus

- [ ] Déploiement Triton dockerisé (P1)
- [ ] Quantization 4-bit pour optimiser les perfs (P1/P3)
- [x] Tests de prompt injection documentés (P4)
